...
class CRF(nn.Module):
    ...
    Args:
        nb_labels (int): number of labels in your tagset, including special symbols (bos_tag_id, eos_tag_id, pad_tag_id).
    ...
    def log_likelihood(self, emissions, tags, mask=None):
        ...
        Args:
            ...
            tags (torch.LongTensor): Sequence of labels.
                ...
                Note: 'tags' do not contain ["BOS","EOS"]
    ...
    def _compute_scores(self, emissions, tags, mask):
        ...
        scores += e_scores + t_scores  # this is bos_to_first_tage score
    ...
    def _compute_log_partition(self, emissions, mask):
        ...
        alphas = self.transitions[self.BOS_TAG_ID, :].unsqueeze(0) + emissions[:, 0]  #(batch_size,nb_labels)
    ...
    def _viterbi_decode(self, emissions, mask):
        ...
        backpointers = []  #(seq_length,batch_size)
        ...
        for i in range(1, seq_length):
            ...
            for tag in range(nb_labels):
                ...
                max_score, max_score_tag = torch.max(scores, dim=-1) # max_score_tag: (batch_size,)
            ...
            backpointers.append(backpointers_t) # backpointers_t:(nb_labels,batch_size)

        # add the scores for the final transition
        last_transition = self.transitions[:, self.EOS_TAG_ID]
        end_scores = alphas + last_transition.unsqueeze(0) # (batch_size,nb_labels)

        # get the final most probable score and the final most probable tag
        max_final_scores, max_final_tags = torch.max(end_scores, dim=1) # (batch_size,)
        ...
        for i in range(batch_size):
            ...
            sample_length = emission_lengths[i].item() # int

            # recover the max tag for the last timestep
            sample_final_tag = max_final_tags[i].item()   #int
            ...
            sample_backpointers = backpointers[: sample_length - 1] #(sample_length,nb_labels,batch_size)
    ...
    def _find_best_path(self, sample_id, best_tag, backpointers):
        """Auxiliary function to find the best path sequence for a specific sample.
            ...
            Args:
                ...
                shape (seq_len_i, nb_labels, batch_size) where seq_len_i
